...
public sealed class LevelSignals
{
    ...
    public event Action<State> StateExited, StateEntered;
    ...
    internal void RaiseStateExited(State state) => StateExited?.Invoke(state);
    internal void RaiseStateEntered(State state) => StateEntered?.Invoke(state);
}